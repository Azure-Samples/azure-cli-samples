...
dmzSubnet="msdocs-dmz-virtual-network-$randomIdentifier"
...
echo "Creating $dmzSubnet"
az network vnet subnet create \
  --address-prefix $dmzPrefix \
  --name $dmzSubnet \
...
az network public-ip create \
  --resource-group $resourceGroup \
  --name $publicIpFirewall

# Create a NIC for the firewall VM and enable IP forwarding.
echo "Creating $nicFirewall with IP forwardimg"
az network nic create \
  --resource-group $resourceGroup \
  --name $nicFirewall \
  --vnet-name $vNet \
  --subnet $dmzSubnet \
  --public-ip-address $publicIpFirewall \
  --ip-forwarding
...
az network route-table route create \
  --name $routeToBackEnd \
  --resource-group $resourceGroup \
  --route-table-name $routeTableFrontEndSubnet \
...
echo "Creating route $routeToInternetFrontEnd to $routeToInternetPrefix in $routeTableFrontEndSubnet"
az network route-table route create \
  --name $routeToInternetFrontEnd \
  --resource-group $resourceGroup \
  --route-table-name $routeTableFrontEndSubnet \
  --address-prefix $routeToInternetPrefix \
  --next-hop-type VirtualAppliance \
  --next-hop-ip-address $Fw1Ip
...
az network route-table route create \
  ...
  --resource-group $resourceGroup \
  --route-table-name $routeTableBackEndSubnet \
  --address-prefix $subnetPrefixBackEnd \
  --next-hop-type VirtualAppliance \
  --next-hop-ip-address $Fw1Ip

# Create a route for traffic from the back-end subnet to the Internet through the firewall VM.
echo "Creating route $routeToInternetBackEnd to $routeToInternetPrefix in $routeTableBackEndSubnet"
az network route-table route create \
  --name $routeToInternetBackEnd \
  --resource-group $resourceGroup \
  --route-table-name $routeTableBackEndSubnet \
  --address-prefix $routeToInternetPrefix \
  --next-hop-type VirtualAppliance \
