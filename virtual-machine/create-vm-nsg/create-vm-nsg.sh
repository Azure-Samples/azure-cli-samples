...
az group create --name myResourceGroup5 --location westeurope

# Create a virtual network and subnet (front end).
az network vnet create --resource-group myResourceGroup5 --name myVnet --address-prefix 192.168.0.0/16 \
--subnet-name mySubnetFrontEnd --subnet-prefix 192.168.1.0/24

# Create a subnet (back end) and associate with virtual network. 
az network vnet subnet create --resource-group myResourceGroup5 --vnet-name myVnet \
  --name mySubnetBackEnd --address-prefix 192.168.2.0/24

# Create a virtual machine. 
az vm create --resource-group myResourceGroup5 --name myVMFrontEnd --image UbuntuLTS \
...
az vm create --resource-group myResourceGroup5 --name myVMBackEnd --image UbuntuLTS \
...
az network nsg rule update --resource-group myResourceGroup5 --nsg-name myNetworkSecurityGroupBackEnd \
  --name default-allow-ssh --protocol tcp --direction inbound --priority 1000 \
...
az network vnet subnet update --resource-group myResourceGroup5 --vnet-name myVnet \
