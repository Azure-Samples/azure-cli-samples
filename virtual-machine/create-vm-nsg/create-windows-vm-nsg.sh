...
az group create --name myResourceGroupNSG2 --location westeurope

# Create a virtual network and subnet (front end).
az network vnet create --resource-group myResourceGroupNSG2 --name myVnet --address-prefix 192.168.0.0/16 \
--subnet-name mySubnetFrontEnd --subnet-prefix 192.168.1.0/24

# Create a subnet (back end) and associate with virtual network. 
az network vnet subnet create --resource-group myResourceGroupNSG2 --vnet-name myVnet \
  --name mySubnetBackEnd --address-prefix 192.168.2.0/24

# Create a virtual machine. 
az vm create --resource-group myResourceGroupNSG2 --name myVMFrontEnd --image win2016datacenter \
...
az vm create --resource-group myResourceGroupNSG2 --name myVMBackEnd --image win2016datacenter \
...
# Get nsg rule name.
nsgrule=$(az network nsg rule list --resource-group myResourceGroupNSG2 --nsg-name myNetworkSecurityGroupBackEnd --query [0].name -o tsv)

# Update backend network security group rule to limit source prefix.
az network nsg rule update --resource-group myResourceGroupNSG2 --nsg-name myNetworkSecurityGroupBackEnd \
