...
az network lb create --resource-group $resourceGroupName --location $location \
  --name myLoadBalancer --public-ip-address myPublicIP \
  --frontend-ip-name myFrontEndPool --backend-pool-name myBackEndPool

# Creates an NLB probe on port 80.
az network lb probe create --resource-group $resourceGroupName --lb-name myLoadBalancer \
  --name myHealthProbe --protocol tcp --port 80 --interval 15 --threshold 4

# Creates an NLB rule for port 80.
az network lb rule create --resource-group $resourceGroupName --lb-name myLoadBalancer \
...
for i in `seq 1 3`; do
  az network nic create --resource-group $resourceGroupName --location $location --name myNic$i \
...
az vm availability-set create --resource-group $resourceGroupName --location $location \
